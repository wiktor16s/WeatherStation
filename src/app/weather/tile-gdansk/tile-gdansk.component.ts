...
import { GdanskServiceService } from './gdansk-service.service';

@Component({
  selector: 'ws-tile-gdansk',
  templateUrl: './tile-gdansk.component.html',
  styleUrls: ['./tile-gdansk.component.css']
})
export class TileGdanskComponent implements OnInit {
  ...
  constructor(private GdanskServiceService : GdanskServiceService) { }

  ngOnInit() {
    if(GdanskServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.GdanskServiceService.getWeather().subscribe((weather) =>{
