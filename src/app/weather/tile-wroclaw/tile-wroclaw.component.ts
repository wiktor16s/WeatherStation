...
import { WroclawServiceService } from './wroclaw-service.service';


@Component({
  selector: 'ws-tile-wroclaw',
  templateUrl: './tile-wroclaw.component.html',
  styleUrls: ['./tile-wroclaw.component.css']
})
export class TileWroclawComponent implements OnInit {
  ...
  constructor(private WroclawServiceService : WroclawServiceService) { }

  ngOnInit() {
    if(WroclawServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.WroclawServiceService.getWeather().subscribe((weather) =>{
