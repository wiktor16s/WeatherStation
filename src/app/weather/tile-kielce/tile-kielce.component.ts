...
import { KielceServiceService } from './kielce-service.service';

@Component({
  selector: 'ws-tile-kielce',
  templateUrl: './tile-kielce.component.html',
  styleUrls: ['./tile-kielce.component.css']
})
export class TileKielceComponent implements OnInit {
  ...
  constructor(private KielceServiceService : KielceServiceService) { }

  ngOnInit() {
    if(KielceServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.KielceServiceService.getWeather().subscribe((weather) =>{
