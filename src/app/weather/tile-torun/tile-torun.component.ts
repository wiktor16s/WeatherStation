...
import { TorunServiceService } from './torun-service.service';

@Component({
  selector: 'ws-tile-torun',
  templateUrl: './tile-torun.component.html',
  styleUrls: ['./tile-torun.component.css']
})
export class TileTorunComponent implements OnInit {
  ...
  constructor(private TorunServiceService : TorunServiceService) { }

  ngOnInit() {
    if(TorunServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.TorunServiceService.getWeather().subscribe((weather) =>{
