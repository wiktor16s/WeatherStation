...
import { CracowServiceService } from './cracow-service.service';
import { WeatherInterface } from '../models/weatherModels';

@Component({
  selector: 'ws-tile-cracow',
  templateUrl: './tile-cracow.component.html',
  styleUrls: ['./tile-cracow.component.css']
})
export class TileCracowComponent implements OnInit {
  ...
  constructor(private CracowServiceService : CracowServiceService) { }

  ngOnInit() {
    if(CracowServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.CracowServiceService.getWeather().subscribe((weather) =>{
