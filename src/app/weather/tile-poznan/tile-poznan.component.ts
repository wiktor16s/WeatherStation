...
import { PoznanServiceService } from './poznan-service.service';

@Component({
  selector: 'ws-tile-poznan',
  templateUrl: './tile-poznan.component.html',
  styleUrls: ['./tile-poznan.component.css']
})
export class TilePoznanComponent implements OnInit {
  ...
  constructor(private PoznanServiceService : PoznanServiceService) { }

  ngOnInit() {
    if(PoznanServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.PoznanServiceService.getWeather().subscribe((weather) =>{
