...
import { KatowiceServiceService } from './katowice-service.service';

@Component({
  selector: 'ws-tile-katowice',
  templateUrl: './tile-katowice.component.html',
  styleUrls: ['./tile-katowice.component.css']
})
export class TileKatowiceComponent implements OnInit {
  ...
  constructor(private KatowiceServiceService : KatowiceServiceService) { }

  ngOnInit() {
    if(KatowiceServiceService){
      this.loadWeather();
    }
  }

  loadWeather() : void {
    this.KatowiceServiceService.getWeather().subscribe((weather) =>{
